...
export function evaluateCondition(
  ...
  const condType = ((condition as any).field || condition.type || "").toLowerCase().trim();
  const condOp = condition.operator as string;
  ...
  switch (condType) {
    ...
    case "queryparam":
    case "query_param": {
      const paramName = (condition.paramName || (condition as any).key || "").toLowerCase();
      if (!paramName) return false;

      const foundValue = context.queryParams ? context.queryParams[paramName] : undefined;
      if (condOp === "exists") {
        return foundValue !== undefined && foundValue !== null && foundValue !== "";
      }
      if (condOp === "not_exists") {
        return foundValue === undefined || foundValue === null || foundValue === "";
      }
  ...
  // If operator is not_exists on a general field
  if (condOp === "not_exists") {
  ...
  switch (condOp) {
    ...
    case "between": {
      // Handles time or time_window ["09:00", "17:00"] or overnight ["22:00", "06:00"]
      if ((condType === "time" || condType === "time_window") && Array.isArray(targetValue) && targetValue.length === 2) {
      ...
      if (condType === "date_range" && Array.isArray(targetValue) && targetValue.length === 2) {
        const current = String(contextValue);
        const [start, end] = targetValue.map(String);
        return current >= start && current <= end;
      }
